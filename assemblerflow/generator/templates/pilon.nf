...
process pilon_{{ pid }} {
    ...
    """
    {
        ...
        java -jar -Xms256m -Xmx\${pilon_mem} /NGStools/pilon-1.22.jar --genome $assembly --frags $bam_file --output ${assembly.take(${assembly.lastIndexOf(".")})}_polished.assembly --changes --threads $task.cpus >> .command.log 2>&1
        echo pass > .status
    } || {
        echo fail > .status
    }
