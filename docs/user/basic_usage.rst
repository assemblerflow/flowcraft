...
.. note::
    However, in a cluster or server environment the ``-sc`` flag can be used
    while building the pipeline in order to use ``SINGULARITY_CACHEDIR`` or
    ``NXF_SINGULARITY_CACHEDIR`` environmental variables instead of the
    FlowCraft default cache dir. This will write a line in ``custom.config``
    with the new ``singularity.cacheDir``, overriding the FlowCraft default
    directory.
